                ...
                <div class="d-flex align-items-center">
                  <div class="dropdown">
                    <a href="#" class="d-flex align-items-center text-decoration-none dropdown-toggle" id="userDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                      <img src="https://placehold.co/100x100" alt="User Avatar" class="rounded-circle me-2" width="32" height="32">
                      <span class="d-none d-sm-inline">{{ user.name }}</span>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="userDropdown">
                      <li><a class="dropdown-item active" href="#"><i class="bi bi-person me-2"></i>Meu Perfil</a></li>
                      <li><a class="dropdown-item" href="#"><i class="bi bi-ticket-perforated me-2"></i>Meus Ingressos</a></li>
            ...
            <div class="row align-items-center">
              <div class="col-md-3 text-center text-md-start mb-4 mb-md-0">
                <div class="position-relative d-inline-block">
                  <img :src="user.avatar" alt="User Avatar" class="rounded-circle img-thumbnail" width="180" height="180">
                  <button class="btn btn-sm btn-light position-absolute bottom-0 end-0 rounded-circle p-2" title="Alterar foto">
              ...
              <div class="col-md-9 text-center text-md-start">
                <div class="d-flex flex-column flex-md-row justify-content-md-between align-items-md-center mb-3">
                  <div>
                    ...
                    <span class="badge bg-light text-primary">{{ user.type }}</span>
                  </div>
          ...
          <div class="container">
            <div class="row g-4">
                  ...
                  <div class="card-body">
                    ...
                    <div class="d-flex align-items-center mb-3">
                      <div class="progress flex-grow-1 me-3" style="height: 8px;">
                        <div class="progress-bar bg-success" role="progressbar" :style="`width: ${user.profileCompletion}%`" :aria-valuenow="user.profileCompletion" aria-valuemin="0" aria-valuemax="100"></div>
                      </div>
                      <span class="text-muted small">{{ user.profileCompletion }}%</span>
                    </div>
                    <p class="card-text small text-muted">Complete seu perfil para melhorar suas recomendações e experiência na plataforma.</p>
                    <div v-if="user.profileCompletion < 100">
                      ...
                      <div class="d-flex align-items-center text-muted mb-2 small">
                        <i class="bi" :class="user.emailVerified ? 'bi-check-circle-fill text-success' : 'bi-circle text-muted'"></i>
                        <span>Verificar e-mail</span>
                      </div>
                      <div class="d-flex align-items-center text-muted mb-2 small">
                        <i class="bi" :class="user.phoneVerified ? 'bi-check-circle-fill text-success' : 'bi-circle text-muted'"></i>
                        <span>Adicionar telefone</span>
                      </div>
                      <div class="d-flex align-items-center text-muted small">
                        <i class="bi" :class="user.preferencesSet ? 'bi-check-circle-fill text-success' : 'bi-circle text-muted'"></i>
              ...
              <div class="col-lg-9">
                    ...
                    <form @submit.prevent="saveProfile">
                      <div class="row g-3">
                        <div class="col-md-6">
                          ...
                          <input type="text" class="form-control" id="name" v-model="user.name" :disabled="!editMode">
                        </div>
                        <div class="col-md-6">
                          <label for="email" class="form-label">Email</label>
                          <div class="input-group">
                            <input type="email" class="form-control" id="email" v-model="user.email" :disabled="!editMode">
                            <span v-if="user.emailVerified" class="input-group-text bg-success text-white" title="Email verificado">
                              <i class="bi bi-check-circle-fill"></i>
                            </span>
                            <span v-else class="input-group-text bg-warning text-white" title="Email não verificado">
                        ...
                        <div class="col-md-6">
                          ...
                          <div class="input-group">
                            <input type="tel" class="form-control" id="phone" v-model="user.phone" :disabled="!editMode">
                            <span v-if="user.phoneVerified" class="input-group-text bg-success text-white" title="Telefone verificado">
                              <i class="bi bi-check-circle-fill"></i>
                            </span>
                            <span v-else class="input-group-text bg-warning text-white" title="Telefone não verificado">
                              <i class="bi bi-exclamation-circle-fill"></i>
                            </span>
                          </div>
                        </div>
                        <div class="col-md-6">
                          ...
                          <input type="date" class="form-control" id="birthdate" v-model="user.birthdate" :disabled="!editMode">
                        </div>
                        <div class="col-md-6">
                          <label for="gender" class="form-label">Gênero</label>
                          <select class="form-select" id="gender" v-model="user.gender" :disabled="!editMode">
                        ...
                        <div class="col-md-6">
                          ...
                          <input type="text" class="form-control" id="cpf" v-model="user.cpf" :disabled="!editMode">
                        </div>
                        <div class="col-12">
                          <label for="address" class="form-label">Endereço</label>
                          <input type="text" class="form-control" id="address" v-model="user.address" :disabled="!editMode">
                        </div>
                        <div class="col-md-4">
                          <label for="city" class="form-label">Cidade</label>
                          <input type="text" class="form-control" id="city" v-model="user.city" :disabled="!editMode">
                        </div>
                        <div class="col-md-4">
                          <label for="state" class="form-label">Estado</label>
                          <select class="form-select" id="state" v-model="user.state" :disabled="!editMode">
                            <option value="">Selecione</option>
                        ...
                        <div class="col-md-4">
                          ...
                          <input type="text" class="form-control" id="zipcode" v-model="user.zipcode" :disabled="!editMode">
                        </div>
                ...
                <!-- Tickets -->
                <div v-if="activeTab === 'tickets'" class="card border-0 shadow-sm mb-4">
                  <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">Meus Ingressos</h5>
                    <div class="btn-group">
                      <button type="button" class="btn btn-sm btn-outline-secondary active">Ativos</button>
                      <button type="button" class="btn btn-sm btn-outline-secondary">Passados</button>
                    </div>
                  </div>
                  <div class="card-body p-0">
                    <div v-if="tickets.length === 0" class="text-center py-5">
                      <i class="bi bi-ticket-perforated text-muted fs-1"></i>
                      <p class="mt-3 mb-0 text-muted">Você ainda não possui ingressos ativos.</p>
                      <a href="#" class="btn btn-primary mt-3">Explorar Eventos</a>
                    </div>
                    <div v-else class="list-group list-group-flush">
                      <div v-for="(ticket, index) in tickets" :key="index" class="list-group-item p-3">
                        <div class="row g-3 align-items-center">
                          <div class="col-md-2">
                            <img :src="ticket.eventImage" :alt="ticket.eventName" class="img-fluid rounded">
                          </div>
                          <div class="col-md-6">
                          ...
                          <div class="col-md-2">
                            <span class="badge" :class="getTicketStatusClass(ticket.status)">{{ ticket.status }}</span>
                            <div class="small text-muted mt-1">{{ ticket.type }}</div>
                          </div>
                          <div class="col-md-2 text-end">
                            <button class="btn btn-sm btn-primary mb-2 w-100">Ver Ingresso</button>
                            <button class="btn btn-sm btn-outline-secondary w-100">Transferir</button>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div v-if="activeTab === 'history'" class="card border-0 shadow-sm mb-4">
                  ...
                  <div class="card-body p-0">
                    <div v-if="eventsHistory.length === 0" class="text-center py-5">
                      <i class="bi bi-calendar-x text-muted fs-1"></i>
                      <p class="mt-3 mb-0 text-muted">Você ainda não participou de nenhum evento.</p>
                      <a href="#" class="btn btn-primary mt-3">Explorar Eventos</a>
                    </div>
                    <div v-else class="list-group list-group-flush">
                      <div v-for="(event, index) in eventsHistory" :key="index" class="list-group-item p-3">
                        <div class="row g-3 align-items-center">
                          <div class="col-md-2">
                            <img :src="event.image" :alt="event.name" class="img-fluid rounded">
                          </div>
                          <div class="col-md-7">
                          ...
                          <div class="col-md-3 text-end">
                            <button class="btn btn-sm btn-outline-primary mb-2 w-100">Ver Detalhes</button>
                            <button class="btn btn-sm btn-outline-secondary w-100">Avaliar Evento</button>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div v-if="activeTab === 'favorites'" class="card border-0 shadow-sm mb-4">
                  ...
                  <div class="card-body p-0">
                    <div v-if="favoriteEvents.length === 0" class="text-center py-5">
                      <i class="bi bi-heart text-muted fs-1"></i>
                      <p class="mt-3 mb-0 text-muted">Você ainda não adicionou eventos aos favoritos.</p>
                      <a href="#" class="btn btn-primary mt-3">Explorar Eventos</a>
                    </div>
                    <div v-else>
                      <div class="row g-4 p-3">
                        <div v-for="(event, index) in favoriteEvents" :key="index" class="col-md-6">
                          <div class="card h-100 border hover-card">
                            <div class="position-relative">
                              <img :src="event.image" :alt="event.name" class="card-img-top">
                              <span class="position-absolute top-0 end-0 badge bg-primary m-2">{{ event.category }}</span>
                              <button class="btn btn-sm btn-light position-absolute top-0 start-0 m-2 rounded-circle" title="Remover dos favoritos">
                            ...
                            <div class="card-body">
                              ...
                              <div class="d-flex justify-content-between align-items-center">
                                <div class="fw-bold">{{ event.price === 0 ? 'Grátis' : `R$ ${event.price.toFixed(2)}` }}</div>
                  ...
                  <div class="card-body">
                    <form @submit.prevent="savePreferences">
                        ...
                        <div class="row g-3">
                          <div v-for="(category, index) in categories" :key="index" class="col-md-4">
                            <div class="form-check">
                              <input class="form-check-input" type="checkbox" :id="`category-${index}`" v-model="userPreferences.categories" :value="category.id" :disabled="!editMode">
                      ...
                      <div class="mb-4">
                        ...
                        <div class="row g-3">
                          <div class="col-md-6">
                            <select class="form-select" v-model="userPreferences.preferredCity" :disabled="!editMode">
                          ...
                          <div class="col-md-6">
                            <div class="form-check">
                              <input class="form-check-input" type="checkbox" id="nearbyEvents" v-model="userPreferences.showNearbyEvents" :disabled="!editMode">
                        ...
                        <div class="row g-3">
                          <div class="col-md-6">
                            <div class="form-check form-switch mb-2">
                              <input class="form-check-input" type="checkbox" id="emailNotifications" v-model="userPreferences.emailNotifications" :disabled="!editMode">
                              <label class="form-check-label" for="emailNotifications">
                                Receber notificações por email
                              </label>
                            </div>
                            <div class="form-check form-switch mb-2">
                              <input class="form-check-input" type="checkbox" id="smsNotifications" v-model="userPreferences.smsNotifications" :disabled="!editMode">
                              <label class="form-check-label" for="smsNotifications">
                                Receber notificações por SMS
                              </label>
                            </div>
                            <div class="form-check form-switch">
                              <input class="form-check-input" type="checkbox" id="pushNotifications" v-model="userPreferences.pushNotifications" :disabled="!editMode">
                          ...
                          <div class="col-md-6">
                            <div class="form-check mb-2">
                              <input class="form-check-input" type="checkbox" id="newEventsNotification" v-model="userPreferences.notifyNewEvents" :disabled="!editMode">
                              <label class="form-check-label" for="newEventsNotification">
                                Novos eventos de interesse
                              </label>
                            </div>
                            <div class="form-check mb-2">
                              <input class="form-check-input" type="checkbox" id="promotionsNotification" v-model="userPreferences.notifyPromotions" :disabled="!editMode">
                              <label class="form-check-label" for="promotionsNotification">
                                Promoções e descontos
                              </label>
                            </div>
                            <div class="form-check">
                              <input class="form-check-input" type="checkbox" id="reminderNotification" v-model="userPreferences.notifyReminders" :disabled="!editMode">
                      ...
                      <div class="mb-4">
                        ...
                        <div class="form-check mb-2">
                          <input class="form-check-input" type="checkbox" id="publicProfile" v-model="userPreferences.publicProfile" :disabled="!editMode">
                          <label class="form-check-label" for="publicProfile">
                            Perfil público (outros usuários podem ver seus eventos)
                          </label>
                        </div>
                        <div class="form-check">
                          <input class="form-check-input" type="checkbox" id="shareActivity" v-model="userPreferences.shareActivity" :disabled="!editMode">
                    ...
                    <form @submit.prevent="changePassword">
                      <div class="mb-4">
                        ...
                        <div class="row g-3">
                          <div class="col-md-12">
                            ...
                            <input type="password" class="form-control" id="currentPassword" v-model="securityForm.currentPassword" :disabled="!editMode">
                          </div>
                          <div class="col-md-6">
                            <label for="newPassword" class="form-label">Nova Senha</label>
                            <input type="password" class="form-control" id="newPassword" v-model="securityForm.newPassword" :disabled="!editMode">
                          </div>
                          <div class="col-md-6">
                            <label for="confirmPassword" class="form-label">Confirmar Nova Senha</label>
                            <input type="password" class="form-control" id="confirmPassword" v-model="securityForm.confirmPassword" :disabled="!editMode">
                          </div>
                        </div>
                      ...
                      <div class="mb-4">
                        ...
                        <div class="form-check form-switch mb-3">
                          <input class="form-check-input" type="checkbox" id="twoFactorAuth" v-model="securityForm.twoFactorEnabled" :disabled="!editMode">
                          <label class="form-check-label" for="twoFactorAuth">
                            Ativar verificação em duas etapas
                          </label>
                        </div>
                        <p class="small text-muted">
                          A verificação em duas etapas adiciona uma camada extra de segurança à sua conta. Além da senha, você precisará fornecer um código enviado para seu telefone ou email.
                        </p>
                        <button v-if="editMode && securityForm.twoFactorEnabled" type="button" class="btn btn-outline-primary btn-sm">Configurar Verificação</button>
                      </div>
  
                      <div class="mb-4">
                        <h6>Sessões Ativas</h6>
                        <div class="list-group mb-3">
                          <div v-for="(session, index) in activeSessions" :key="index" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                            <div>
                              <div class="d-flex align-items-center">
                                <i :class="`bi ${getDeviceIcon(session.device)} me-2`"></i>
                                <div>
                                  <div>{{ session.device }}</div>
                                  <div class="small text-muted">{{ session.location }} • {{ session.lastActive }}</div>
                                </div>
                              </div>
                            </div>
                            <button v-if="session.current" class="badge bg-primary border-0">Sessão Atual</button>
                            <button v-else type="button" class="btn btn-sm btn-outline-danger" @click="terminateSession(index)">Encerrar</button>
                          </div>
                        </div>
                        <button type="button" class="btn btn-outline-danger btn-sm" @click="terminateAllSessions">Encerrar Todas as Sessões</button>
                      </div>
                          ...
                          <table class="table table-hover">
                            ...
                            <tbody>
                              <tr v-for="(login, index) in loginHistory" :key="index">
                                ...
                                <td>
                                  <span class="badge" :class="login.success ? 'bg-success' : 'bg-danger'">
    ...
    data() {
      return {
        ...
        tabs: [
          ...
          { id: 'history', name: 'Histórico de Eventos', icon: 'bi-clock-history' },
        ...
        cities: [
          ...
          'Brasília', 'Curitiba', 'Fortaleza', 'Recife', 'Porto Alegre'
        ],
        userPreferences: {
